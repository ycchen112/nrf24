...
void Nrf24_setAutoAck(NRF24_t * dev, bool enable) {
    if (enable) {
        Nrf24_writeRegister(dev, EN_AA, 0x3F);
    } else {
        Nrf24_writeRegister(dev, EN_AA, 0x00);
    }

void Nrf24_disableCRC(NRF24_t * dev) {
    uint8_t config;
    Nrf24_readRegister(dev, CONFIG, &config, 1); // 读取 CONFIG 寄存器
    config &= ~(1 << EN_CRC); // 禁用 CRC
    Nrf24_configRegister(dev, CONFIG, config); // 写回 CONFIG 寄存器
}

void Nrf24_stopListen(NRF24_t * dev) {
	uint8_t config;
    Nrf24_readRegister(dev, CONFIG, &config, 1); // 读取 CONFIG 寄存器
    config &= ~(1 << PWR_UP); // 发送模式
    Nrf24_configRegister(dev, CONFIG, config); // 写回 CONFIG 寄存器
}

void Nrf24_setRetries(NRF24_t * dev, uint8_t delay, uint8_t count) {
    uint8_t setup_retr = (delay << 4) | (count & 0x0F); // 组合延迟和重试次数
    Nrf24_configRegister(dev, SETUP_RETR, setup_retr);
}

void Nrf24_setChannel(NRF24_t * dev, uint8_t channel) {
    // `channel` 的值应在 0 到 125 之间
    if (channel > 125) {
        channel = 125;  // 限制最大值为 125
    }
    
    // 将频道写入 RF_CH 寄存器
    Nrf24_writeRegister(dev, RF_CH, channel);
}

// 启动恒定载波
void NRF24_startConstCarrier(NRF24_t * dev, uint8_t powerLevel, uint8_t channel) {
    uint8_t rf_setup;

	Nrf24_setChannel(dev, channel);
	Nrf24_SetOutputRF_PWR(dev, RF24_PA_MAX);

    Nrf24_readRegister(dev, RF_SETUP, &rf_setup, 1);	// 读取 RF_SETUP 寄存器的值
    
    // 设置 RF_PWR 和 PLL_LOCK
    rf_setup |= (1 << 7) | (1 << 4); // 设置 PLL_LOCK 和最高功率
    Nrf24_writeRegister(dev, RF_SETUP, rf_setup);		// 写回 RF_SETUP 寄存器
}
